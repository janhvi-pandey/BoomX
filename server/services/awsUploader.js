...
const s3Client = new S3Client({
  region: process.env.AWS_REGION_BOOMX,
  credentials: {
    accessKeyId: process.env.AWS_ACCESS_KEY_BOOMX,
    secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY_BOOMX,
  },
});
...
const uploadToS3 = async (filePath, type) => {
  ...
  const uploadParams = {
    Bucket: process.env.AWS_BUCKET_NAME_BOOMX,
    Key: uniqueFileName,
    Body: fileStream,
